...
export function DateTimeRangePicker( start_id, end_id, settings = {} ) {
  ...
  const change_handler = ( typeof settings.change_handler === 'function' ) ? settings.change_handler : null;
  const styles = ( settings.styles ) ? settings.styles : {};
  this.setStartPickerProps( start_id, start_date, first_date, last_date, first_day_no, change_handler );
